...
export default class Chart extends React.Component {
  constructor(props) {
    ...
    super(props);
    this.state = { graphID: props.graph_id };
    this.chart = null;
    this.wrapper = null;
    this.hidden_columns = {};
    this.dataTable = [];
  ...
  componentWillUnmount() {
    ...
    try {
      ...
      debug('Chart::componentWillUnmount', err);
    }
  }
  ...
  buildDataTableFromProps() {
    ...
    if (this.props.diffdata) {
      ...
      const { computeDiff } =
                window.google.visualization[this.props.chartType].prototype;
    ...
    this.props.formatters.forEach(({ type, column, options }) => {
      switch (type) {
        ...
        case 'DateFormat':
          ...
          break;
        default:
          debug('Unkown formatter type: %s', type);
          break;
      }
    });
  ...
  updateDataTable() {
    ...
    window.google.visualization.errors
      .removeAll(document.getElementById(this.wrapper.getContainerId()));
